...
public class ControlJuego {
	...
	public void inicializarPartida() {
		...
		for (int i = 0; i < MINAS_INICIALES; i++) {
			...
			// no hay minas colocamos una mina, y si la ahi volvemos a generar los
			// aleatorios.
	...
	private int calculoMinasAdjuntas(int i, int j) {
		...
		int cont = 0;
		// nos declaraos una variables para ver donde empieza y acaba nuestro tablero
		// (vertical, horizontal).
				...
				if (iIni >= 0 && iIni <= LADO_TABLERO - 1) {
					if (jIni >= 0 && jIni <= LADO_TABLERO - 1) {
						...
						if (tablero[iIni][jIni] == MINA) {
							// si o es aumentamos el contador y asi sabremos cuantas minas ahi al lado de
							// nuestra celda.
							cont++;
						}
					}
	...
	/**
	 * Método que nos permite abrir una casilla, retorna verdadero y suma 1 punto si
	 * no hay mina.
	 * 
